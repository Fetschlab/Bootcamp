...
%%  1.4 - sidebar: comparing accuracy alone can be misleading

clearvars -except cohs cohsInterp
close all
clc
...
% simulate some data with steep slope but also a large bias
Beta2 = [2.5 18]';
scoh = nan(ntrials,1);
...
figure(2); set(gcf,'Color',[1 1 1],'Position',[300 500 700 550]);
plot(cohs,pRight1(:,1),'ko-','MarkerSize',9);
xlabel('Motion strength (%coh)');
ylabel('Proportion rightward choices');
changeAxesFontSize(gca,15,15);
...
% now simulate unbiased choices but with shallower slope (less sensitivity)
Beta2 = [0 8]';
scoh = nan(ntrials,1);
choice = nan(ntrials,1);
for t = 1:ntrials
...
figure(2); hold on; plot(cohs,pRight2(:,1),'rs-','MarkerSize',9);
changeAxesFontSize(gca,15,15);

% accuracy (percent correct) will be about the same (I chose the parameters
% to make it this way), but clearly this does not tell the whole story
...
k = 8; % proportionality constant converting our stimulus variable (coh)
       % to the mean of the signal distribution; chosen to approximate the
       % data (higher the value, the greater the sensitivity)
sigma = 1; % s.d. of the signal distribution

% plot the sampling distributions for a subset of cohs
...
% methods, MCMC, BADS, etc). These are beyond of our current scope but ask
